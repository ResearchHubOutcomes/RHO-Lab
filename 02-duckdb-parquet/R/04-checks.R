...
# ---------------------------------------------------------------------------
# (c) the middle road: the CSV queried by DuckDB, without converting anything
# ---------------------------------------------------------------------------
csv <- "data/sdo_large.csv"
src_csv <- tbl(con, sql(sprintf("SELECT * FROM read_csv_auto('%s')", csv)))
...
invisible(readmissions_on(src_csv))                    # warm-up
t_csv <- median(replicate(3, timed(readmissions_on(src_csv))))
cat(sprintf("Riammissioni, CSV interrogato da DuckDB: %.1f s\n", t_csv))
cat("  (in memoria in R: 202,2 s; su parquet: 1,9 s)\n")
