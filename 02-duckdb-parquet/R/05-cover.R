...
# Same two series as the article's chart: the two situations it compares.
strategy_cols <- c(csv_memory = NAVY, duckdb_parquet = ORANGE)
strategy_off  <- c(csv_memory = 3.0, duckdb_parquet = -3.0)
...
d <- big[big$strategy %in% names(strategy_cols), ]
...
illu_02 <- list(
  ...
  legend_row(47, 11,
             c("CSV letto in memoria in R", "Parquet interrogato da DuckDB"),
             c(NAVY, ORANGE)),
